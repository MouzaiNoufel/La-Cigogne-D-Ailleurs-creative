...
(() => {
  ...
  function renderInspector3D(){
    ...
    const old=document.querySelector('.phase3-card'); if(old)old.remove();
    inspectorBody.insertAdjacentHTML('beforeend',`<div class="phase3-card phase3d-card professional-3d-card">
      <div class="phase3-title">3D · ROOM MATCH</div>
      <div class="mode-switch"><button id="p3Photo" class="mode-btn ${state.mode==='photo'?'active':''}">Photo</button><button id="p3Orbit" class="mode-btn ${state.mode==='orbit'?'active':''}">Vue libre</button></div>
      ${item?`<div class="three-selected"><span class="three-dot"></span><div><strong>${item.name}</strong><small>Objet 3D sélectionné</small></div></div>
      <div class="transform-grid"><button id="p3Move" class="transform-btn ${state.transform?.getMode?.()==='translate'?'active':''}"><span>↔</span><small>Déplacer</small></button><button id="p3Rotate" class="transform-btn ${state.transform?.getMode?.()==='rotate'?'active':''}"><span>↻</span><small>Tourner</small></button><button id="p3Scale" class="transform-btn ${state.transform?.getMode?.()==='scale'?'active':''}"><span>⤢</span><small>Taille</small></button></div>
      <div class="quick-placement"><button id="p3Left">←</button><button id="p3Up">↑</button><button id="p3Down">↓</button><button id="p3Right">→</button></div>`:'<p class="hint">Sélectionnez un meuble pour le manipuler en 3D.</p>'}
      <div class="three-status"><span>Occlusion</span><strong>${state.occlusion.enabled&&state.depthFit.ready?'Active':'Prête'}</strong></div>
      <details class="advanced-details"><summary>Réglages avancés</summary>
        <div class="advanced-group"><div class="phase3-subtitle">CAMÉRA</div>
          <div class="field"><label>Hauteur <span id="p3HVal">${state.calibration.height.toFixed(2)} m</span></label><input id="p3H" type="range" min="1.6" max="4.2" step="0.05" value="${state.calibration.height}"></div>
          <div class="field"><label>Perspective <span id="p3FVal">${state.calibration.fov}°</span></label><input id="p3F" type="range" min="35" max="70" step="1" value="${state.calibration.fov}"></div>
          <label class="p3-check"><input id="p3Auto" type="checkbox" ${state.calibration.auto?'checked':''}> Calibration automatique</label>
        </div>
        <div class="advanced-group"><div class="phase3-subtitle">LUMIÈRE</div>
          <label class="p3-check"><input id="p3LightAuto" type="checkbox" ${state.lighting.auto?'checked':''}> Estimer depuis la photo</label>
          <div class="field"><label>Température <span id="p3TempVal">${Math.round(3000+state.lighting.temperature*4000)} K</span></label><input id="p3Temp" type="range" min="0" max="1" step="0.02" value="${state.lighting.temperature}"></div>
          <div class="field"><label>Exposition <span id="p3EVal">${state.lighting.exposure.toFixed(2)}</span></label><input id="p3E" type="range" min="0.70" max="1.45" step="0.01" value="${state.lighting.exposure}"></div>
          <div class="field"><label>Lumière clé <span id="p3KVal">${state.lighting.key.toFixed(2)}</span></label><input id="p3K" type="range" min="0.8" max="4.5" step="0.1" value="${state.lighting.key}"></div>
        </div>
        <div class="advanced-group"><div class="phase3-subtitle">PROFONDEUR & OCCLUSION</div>
          <label class="p3-check"><input id="p3Occl" type="checkbox" ${state.occlusion.enabled?'checked':''}> Occlusion par profondeur IA</label>
          <div class="field"><label>Force <span id="p3OVal">${Math.round(state.occlusion.strength*100)}%</span></label><input id="p3O" type="range" min="0" max="1" step="0.05" value="${state.occlusion.strength}"></div>
          <div class="field"><label>Tolérance <span id="p3TVal">${state.occlusion.threshold.toFixed(2)}</span></label><input id="p3T" type="range" min="0.04" max="0.45" step="0.01" value="${state.occlusion.threshold}"></div>
        </div>
      </details>
    </div>`);
    ...
    if(item){
      ...
      const nudge=(dx,dy)=>{item.x+=dx;item.y+=dy;App.clampToRoom?.(item);syncAll();App.draw();AppActions?.renderInspector?.();renderInspector3D();status('Position mise à jour');};
      q('p3Left').onclick=()=>nudge(-6,0);q('p3Right').onclick=()=>nudge(6,0);q('p3Up').onclick=()=>nudge(0,-6);q('p3Down').onclick=()=>nudge(0,6);
    }
  }
