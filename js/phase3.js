...
(() => {
  ...
  function renderInspector3D(){
    ...
    if(item){
      const nudge=(dx,dy)=>{item.x+=dx;item.y+=dy;App.clampToRoom?.(item);syncAll();App.draw();status('Position précise enregistrée ✔');};
      q('p3Left').onclick=()=>nudge(-10,0);q('p3Right').onclick=()=>nudge(10,0);q('p3Up').onclick=()=>nudge(0,-10);q('p3Down').onclick=()=>nudge(0,10);
      q('p3RMinus').onclick=()=>{item.rot-=Math.PI/36;syncAll();App.draw();renderInspector3D();};q('p3RPlus').onclick=()=>{item.rot+=Math.PI/36;syncAll();App.draw();renderInspector3D();};
      q('p3SMinus').onclick=()=>{item.scale=Math.max(.3,item.scale-.05);syncAll();App.draw();renderInspector3D();};q('p3SPlus').onclick=()=>{item.scale=Math.min(3,item.scale+.05);syncAll();App.draw();renderInspector3D();};
    }
  }
